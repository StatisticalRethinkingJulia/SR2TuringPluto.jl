...
# ╔═╡ a7e2b505-2042-4dd6-a950-d56b509ebfac
md"
!!! note
	In the second edition of Statistical Rethinking, there appears to be a mistake in the normalization step given for the grid approximation.
	When calculating the total area under the curve (by taking the Riemann sum), the width of each grid-slice is not taken into account.
	Though there exist more accurate ways to calculate this integral, here we opt for the most straightforward solution: dividing each `unstd_posterior` by the `size` of the grid before `sum()`ing.
	This should bring the scale of the grid approximation back in line with the other approximations found throughout this chapter, and should prevent the scale of the y-axis from changing every time the size of the grid does! 
"
...
manifest_format = "2.0"
project_hash = "db8cd5afee9c2f9f3a34ee4cdd8fdf8c7fd04061"
